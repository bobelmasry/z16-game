...
export default function CodeWindow({ code, onCodeChange, PC }: CodeWindowProps) {
  ...
  const highlightCurrentInstruction = ViewPlugin.fromClass(
    class {
      ...
      getDecorations(view: { state: { doc: { line: (arg0: number) => any; }; }; }) {
        const lines = code
          .split("\n")
          .map((line: string, index: any) => ({ text: line.split("#")[0].trim(), index }))
          .filter((line: { text: string; }) => line.text !== ""); // Remove empty/comment-only lines
        if (typeof PC !== "number" || PC >= lines.length) return Decoration.none;
...
const z16Language = StreamLanguage.define({
  startState: () => ({}),
  token: (stream: { eatSpace: () => any; match: (arg0: RegExp) => any; next: () => void; }) => {
